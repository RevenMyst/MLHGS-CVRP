...
{	
	...
	{	
		/* SELECTION AND CROSSOVER */
		Individual parent1 = population.getRanked();
		Individual parent2 = population.getRanked();

		std::vector<Individual> offsprings = exhaustiveCrossoverOX(parent1,parent2);
		double best_score = -1;
...
void Genetic::exportBestCuts(std::string fileName, std::string instanceName)
{
	std::ofstream myfile;
	myfile.open(fileName, std::ios_base::app);
	for (std::tuple<std::vector<int>,std::vector<int>,int,int> state : best_cuts){
		myfile << instanceName << ";";
		for(int i = 0; i< std::get<0>(state).size();i++){
			myfile << std::get<0>(state)[i];
			if(i < std::get<0>(state).size()-1){
				myfile << ",";
			}
		}
		myfile << ";";
		for(int i = 0; i< std::get<1>(state).size();i++){
			myfile << std::get<1>(state)[i];
			if(i < std::get<1>(state).size()-1){
				myfile << ",";
			}
		}
		myfile << ";" << std::get<2>(state) << ";" << std::get<3>(state) << std::endl;
	}
